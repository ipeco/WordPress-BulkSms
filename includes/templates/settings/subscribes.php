...
<div class="wrap">
	...
	<h2>
		...
		<?php if(isset($_POST['s'])) { ?><span class="subtitle"><?php echo sprintf(__('Search result for %s', 'wordpress_smsir'), $_POST['s']); ?></span><?php } ?>
	</h2>
	
	
	<ul class="subsubsub">
		<li class="all">
		<a <?php if(isset($_GET['group']) == false) { echo 'class="current" '; } ?>href="admin.php?page=wordpress_smsir/subscribe">
						...
						if($gets->group_ID){
							if (strpos($gets->group_ID, ',') != false) {					
								...
								foreach($groups_id as $g_key=>$g_val){
									$result = $wpdb->get_row("SELECT * FROM {$table_prefix}smsir_subscribes_group WHERE `ID` = '{$g_val}'");
									if($result){
										$group .= " <a href='admin.php?page=wordpress_smsir/subscribe&group={$result->ID}'>{$result->name}</a> ,";
									}
								}
								echo substr($group, 0, -1);
								unset($group);
							} else {
