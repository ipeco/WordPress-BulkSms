				...
				if($verify_config == "on"){
					...
					if($wp_resend){
						...
						if($ReSendSMSforVerification){
							...
							if($Reselect > 0){
								$Reupdate = $wpdb->update("{$table_prefix}smsir_verification",
									array(
										'code' => $random_number,
										'status' => 'pending',
								...
								$Reinsert = $wpdb->insert("{$table_prefix}smsir_verification", array(
									'user_login' => $user_login,
									'user_email' => $user_email,
									'mobile' => $mobile,
									'code' => $random_number,
					...
					if($verification == ""){
						if($wp_submit != "verify"){
							...
							if($SendSMSforVerification){
								...
								if($select > 0){
									$update = $wpdb->update("{$table_prefix}smsir_verification",
										array(
											'code' => $random_number,
											'status' => 'pending',
											'add_time' => $get_current_date,
									...
									$insert = $wpdb->insert("{$table_prefix}smsir_verification", array(
										'user_login' => $user_login,
										'user_email' => $user_email,
										'mobile' => $mobile,
										'code' => $random_number,
